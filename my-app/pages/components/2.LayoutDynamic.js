...
export default function LayoutDynamic(props) {
    ...
    return (
        <>
            ...
            <Link href={"/dashboard"}>Dashboard (index)</Link> | 
            <Link href={"/dashboard/preferences"}>Dashboard (preferences)</Link> | 
            <Link href={"/readingQueryParamProducts?page=1&category=stationary&category=office"}>Products</Link> | 
            <Link href={"/post/3"}>Post (3)</Link> | 
            <Link href={"/post/5"}>Post (5)</Link>
            <hr />
            <br />
            {props.children}
            <br /><br />
